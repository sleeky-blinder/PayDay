...
/* ---------- state ---------- */
var state={opening:Object.assign({},DEFAULT_OPEN),settings:Object.assign({},DEFAULT_SET),done:{},subs:null,plan:null,confirmed:{},catLimits:{},rates:{}};
...
function adoptState(s){ state.opening=Object.assign({},DEFAULT_OPEN,s.opening||{}); state.settings=Object.assign({},DEFAULT_SET,s.settings||{}); state.done=s.done||{}; state.subs=Array.isArray(s.subs)?s.subs:null; state.plan=(s.plan&&Array.isArray(s.plan.lines))?s.plan:null; state.confirmed=s.confirmed||{}; state.catLimits=s.catLimits||{}; state.rates=s.rates||{}; }
...
function nv(v){ var n=+String(v==null?'':v).replace(/[, ]/g,''); return isNaN(n)?0:n; }
function grp(v){ var s=String(v==null?'':v).replace(/[^0-9.]/g,''); var i=s.indexOf('.'); if(i>=0) s=s.slice(0,i+1)+s.slice(i+1).replace(/\./g,'');
  var p=s.split('.'); p[0]=p[0].replace(/\B(?=(\d{3})+(?!\d))/g,','); return p[1]!=null?p[0]+'.'+p[1].slice(0,2):p[0]; }
...
function rateOf(id){ var a=ACCMAP[id]; if(!a) return ''; var r=(state.rates||{})[id]; return (r!=null&&r!=='')?r:(a.rate||''); }
...
function nipFee(a){ var s=state.settings; var free=+s.nipFree||0, midTo=+s.nipMidTo||0;
  if(a<=free) return 0; if(a<=midTo) return +s.nipMid||0; return +s.nipHigh||0; }
function feesFor(t){ if(t.fee||t.recon) return []; var m=methodFor(t), c=feeCfg()[t.from]; if(m==='none'||m==='card'||m==='inapp'||m==='cash'||m==='ownatm') return []; if(!c&&m!=='pos'&&m!=='atm') return []; c=c||{}; var out=[]; var what=(t.to==='spend'?(t.note||t.cat||'expense'):name(t.to));
  if(m==='transfer'){ if(c.stamp&&t.amt>=10000) out.push({amt:50,note:'Stamp duty · ₦'+fmt(t.amt,0)+' → '+what}); if(c.nip){ var nf=nipFee(t.amt); if(nf>0) out.push({amt:nf,note:'Transfer fee · → '+what}); } }
...
function prompts(){ var b=balances(), s=state.settings, out=[], day=now.getDate(), run=currentRun();
  ...
  if(bi&&!bi.kept){ var why = bi.spentSince<1 ? 'Nothing has been spent from Pocket since it arrived — add the spend itself as an expense so the month shows it.' : (bi.unspent>=1 ? naira(bi.spentSince,0)+' spent from Pocket since, so about '+naira(bi.unspent,0)+' of it is still unused. Send that back to '+name(bi.src)+', or keep it and let this month have it to spend \u2014 then confirm Pocket against the app.' : 'All of it has gone on spending since; the month has absorbed it. Keep it to fold it into this month\'s allowance.');
...
function moneyFields(root){ var R=root||document;
  [].forEach.call(R.querySelectorAll('input[type=number]'),function(i){ i.setAttribute('type','text'); i.setAttribute('inputmode','decimal'); i.setAttribute('autocomplete','off'); i.dataset.money='1'; i.value=i.value?grp(i.value):''; });
  [].forEach.call(R.querySelectorAll('input[data-money]'),function(i){ if(i!==document.activeElement){ var g=grp(i.value); if(i.value!==g) i.value=g; } }); }
function moneyType(i){ var s=i.selectionStart, old=i.value, before=(old.slice(0,s).match(/[0-9.]/g)||[]).length, v=grp(old);
  if(v!==old){ i.value=v; var k=0,c=0; while(k<v.length&&c<before){ if(/[0-9.]/.test(v[k])) c++; k++; } try{ i.setSelectionRange(k,k); }catch(e){} } }
document.addEventListener('input',function(ev){ var t=ev.target; if(t&&t.dataset&&t.dataset.money==='1') moneyType(t); });
...
function renderPotGroups(b,e){ var u=usdBalances(), s=state.settings, html='', trAll=tracked(b);
  ...
  GROUPS.forEach(function(g){ var list=ACC.filter(function(a){return a.grp===g[0];}); if(!list.length) return; var sum=list.reduce(function(t,a){return t+b[a.id];},0); if(sum===0&&list.every(function(a){return (e[a.id]||0)===0;})&&g[0]!=='Spending') return;
    ...
    list.forEach(function(a){ var v=b[a.id], x=e[a.id], rr0=a.cash?0:returnsFor(a.id), gap=v-rr0-x, st=potStatus(a,v,x,gap); var c=state.confirmed[a.id]; var tgt=a.tgt==='ef'?s.ef:(a.tgt==='rent'?s.rentTarget:null);
      var why=[]; var rr=returnsFor(a.id); if(!a.cash&&Math.abs(rr)>=1){ var base=v-rr; why.push('returns since anchor '+(rr>=0?'+':'−')+naira(Math.abs(rr),0)+(base>0?' ('+(rr>=0?'+':'−')+Math.abs(rr/base*100).toFixed(1)+'%)':'')); } else if(rateOf(a.id)) why.push('quoted '+esc(rateOf(a.id))); if(!a.cash) why.push('plan '+naira(x,0)); if(c) why.push('confirmed '+dLabel(c.d)); if(st[1]) why.push(st[1]);
...
/* balances on a date */
/* pots ref */
function renderPotsRef(b){ el('potsRef').innerHTML=ACC.filter(function(a){return !a.cash;}).concat(ACC.filter(function(a){return a.cash;})).map(function(a){ return '<tr><td><b>'+a.name+'</b>'+(rateOf(a.id)?'<span class="why">quoted '+esc(rateOf(a.id))+'</span>':'')+'</td><td class="small num hide-m">'+esc(a.path)+'</td><td class="small">'+esc(a.job)+'</td></tr>'; }).join('')+
...
function amtsText(o){ return Object.keys(o||{}).sort().map(function(k){return k+': '+grp(o[k]);}).join(', '); }
function parseAmts(s){ var o={}; String(s||'').split(/[;\n]|,(?=\s*\d{4}-\d{2}\s*:)/).forEach(function(p){ var m=/^\s*(\d{4}-\d{2})\s*:\s*([\d.,]+)\s*$/.exec(p); if(m) o[m[1]]=nv(m[2]); }); return Object.keys(o).length?o:null; }
function renderFees(){ var c=feeCfg(); var S=state.settings;
  el('feeBandsTh').textContent='Transfer fee — free to '+naira(S.nipFree,0)+', then '+naira(S.nipMid,0)+' to '+naira(S.nipMidTo,0)+', then '+naira(S.nipHigh,0); var cols=['stamp','nip','sms','stampDebit']; el('feesBody').innerHTML=['zenith','pocket','flex'].map(function(a){ return '<tr><td><b>'+name(a)+'</b></td>'+cols.map(function(k){ if(k==='stampDebit'&&a!=='zenith') return '<td class="small muted">—</td>'; return '<td><input class="chk" type="checkbox" data-fee="'+a+'.'+k+'" aria-label="'+k+' '+a+'"'+(c[a][k]?' checked':'')+'></td>'; }).join('')+'</tr>'; }).join(''); }
...
function renderSettings(){ var s=state.settings; renderFees(); renderBillsEditor(); renderPlanEditor(); if(!el('catBody').children.length) renderCats(); el('sAllow').value=s.allowance; el('sCushion').value=s.cushion; el('sFloat').value=s.zfloat; el('sRate').value=s.rate; el('sEF').value=s.ef; el('sRent').value=s.rentTarget; el('sNipFree').value=s.nipFree; el('sNipMid').value=s.nipMid; el('sNipMidTo').value=s.nipMidTo; el('sNipHigh').value=s.nipHigh; el('sPosFee').value=s.posFee; el('sPosPer').value=s.posPer; el('sAtmFee').value=s.atmFee; el('sAtmPer').value=s.atmPer; el('anchorDate').textContent='As of '+dLabelY(s.anchorDate);
  el('openBody').innerHTML=ACC.map(function(a){ return '<div class="lf"><span class="lbl">'+a.name+(a.app!==a.name?' · '+a.app:'')+'</span>'+
    (a.cash?'<input class="num" type="number" step="0.01" id="open-'+a.id+'" value="'+(+state.opening[a.id]||0)+'" aria-label="Opening '+a.name+'">'
          :'<div class="inline-act"><input class="num" type="number" step="0.01" id="open-'+a.id+'" value="'+(+state.opening[a.id]||0)+'" aria-label="Opening '+a.name+'"><input id="rate-'+a.id+'" value="'+esc(rateOf(a.id))+'" placeholder="rate" aria-label="Quoted rate '+a.name+'"></div>')+'</div>'; }).join('');
  if(!db&&!el('storeWhere').textContent) el('storeWhere').textContent='Saved on this device'; }

/* ---------- edit sheet ---------- */
function openSheet(html){ el('sheet').innerHTML=html; moneyFields(); el('sheet').classList.add('on'); el('sheetBg').classList.add('on'); var f=el('sheet').querySelector('input,select'); if(f) f.focus(); }
...
function hints(){ [].forEach.call(document.querySelectorAll('.card-h > .src.hint'),function(sp){
  var h=sp.parentNode.querySelector('h3'); var t=sp.textContent.replace(/\s+/g,' ').trim(); if(!h||!t) return; sp.parentNode.removeChild(sp);
  var b=document.createElement('button'); b.type='button'; b.className='info'; b.textContent='i'; b.title=t; b.setAttribute('aria-label',t);
...
document.addEventListener('click',function(ev){ var gh=ev.target.closest('.grp[data-grp]'); if(gh){ var open=!gh.classList.contains('open'); gh.classList.toggle('open',open); gh.setAttribute('aria-expanded',open); var body=gh.nextElementSibling; if(body) body.hidden=!open; ls('grp-'+gh.dataset.grp,open?'1':'0'); if(open) renderPotGroups(balances(),expected(TODAY)); return; }
  ...
  else if(d.doit){ var it=findItem(d.doit); if(!it) return; var inp=el('amt-'+it.key); var amt=inp?nv(inp.value):it.amt; if(!(amt>=0)) return toast('Enter an amount'); var tx=postItem(it,amt); toast(name(it.from)+' → '+name(it.to)+' '+naira(amt,0)+feeNote(tx)); }
  ...
  else if(d.keep){ var bk=borrowInfo(d.keep); state.done['keep-borrow-'+d.keep]=true; saveState(); render(); toast((bk?naira(bk.net,0)+' kept. ':'Kept. ')+'Is that really what Pocket holds?','Check Pocket',function(){ openRecon('pocket'); }); }
  ...
  else if(b.id==='btnSaveCats'){ var L={}; document.querySelectorAll('[data-catlim]').forEach(function(i){ if(nv(i.value)>0) L[i.dataset.catlim]=nv(i.value); }); state.catLimits=L; saveState(); render(); toast('Limits saved'); }
...
el('mvForm').addEventListener('submit',function(ev){ ev.preventDefault(); var amt=nv(el('mvAmt').value), f=el('mvFrom').value, t=el('mvTo').value; if(!(amt>0)) return toast('Enter an amount'); if(f===t) return toast('From and To are the same'); var tx=post({d:el('mvDate').value||TODAY,from:f,to:t,amt:amt,note:el('mvNote').value.trim(),method:el('mvHow').value||undefined}); el('mvAmt').value=''; el('mvNote').value=''; toast('Moved '+naira(amt,0)+' · '+name(f)+' → '+name(t)+feeNote(tx)); });
el('setForm').addEventListener('submit',function(ev){ ev.preventDefault(); var s=state.settings; s.allowance=nv(el('sAllow').value)||s.allowance; s.cushion=nv(el('sCushion').value)||0; s.zfloat=nv(el('sFloat').value)||0; s.rate=nv(el('sRate').value)||s.rate; s.ef=nv(el('sEF').value)||s.ef; s.rentTarget=nv(el('sRent').value)||s.rentTarget; s.nipFree=nv(el('sNipFree').value)||0; s.nipMid=nv(el('sNipMid').value)||0; s.nipMidTo=nv(el('sNipMidTo').value)||0; s.nipHigh=nv(el('sNipHigh').value)||0; s.posFee=nv(el('sPosFee').value)||0; s.posPer=nv(el('sPosPer').value)||10000; s.atmFee=nv(el('sAtmFee').value)||0; s.atmPer=nv(el('sAtmPer').value)||20000; saveState(); render(); toast('Saved'); });
el('retSel').addEventListener('change',function(){ state.settings.ret=+el('retSel').value; saveState(); renderGoals(balances()); });
el('btnSaveOpen').addEventListener('click',function(){ state.rates=state.rates||{}; ACC.forEach(function(a){ var v=el('open-'+a.id); if(v) state.opening[a.id]=nv(v.value)||0; var r=el('rate-'+a.id); if(r){ var t=r.value.trim(); if(t&&t!==(a.rate||'')) state.rates[a.id]=t; else delete state.rates[a.id]; } }); saveState(); render(); toast('Balances and rates saved'); });
...
document.addEventListener('click',function(ev){ var i=ev.target.closest&&ev.target.closest('button.info'); if(i&&i.title){ ev.stopPropagation(); toast(i.title); } },true);
...
loadLocal(); fillSelects(); hints(); render(); accordion(el('v-pots'),false); accCard(el('recentCard'),'v-today',true); accCard(el('needsCard'),'v-today',true); accCard(el('billsCard'),'v-today',false); accCard(el('monthEnd'),'v-ledger',false); accordion(el('v-settings'),false); accordion(el('v-plan'),false); show((location.hash||'').replace('#','')||ls('pm-tab')||'today');
...
var APP_VERSION='v20260915-31983';
